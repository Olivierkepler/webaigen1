...
export default function Chatbot() {
  ...
  const [theme, setTheme] = useState<"light" | "dark">("dark"); // Default to dark for this brand
  ...
  const runTypewriter = (text: string, onDone: () => void) => {
    ...
    const step = () => {
      ...
      if (idx < clean.length) {
        ...
        let delay = 15; // Faster for tech feel
          ...
          `}>
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/5 bg-black/20">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className={`font-mono text-xs font-bold uppercase tracking-widest ${isDark ? "text-white" : "text-zinc-900"}`}>
                    AI_Architect
                  </h3>
            ...
            <div className="flex flex-1 overflow-hidden relative">
               ...
               {/* Estimator Panel (Collapsible) */}
               <div className={`transition-all duration-500 ease-[0.22,1,0.36,1] border-r border-white/5 ${
               ...
               <div className="flex-1 flex flex-col relative">
                  <div className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                    {messages.map((m) => {
                        ...
                        return (
                          <div key={m.id} className={`flex flex-col ${isUser ? "items-end" : "items-start"}`}>
                             ...
                             `}>
                                {/* Tiny corner accents */}
                                <div className={`absolute top-0 w-2 h-2 border-t border-l ${isUser ? "right-0 border-[#d4af37]" : "left-0 border-white/40"}`} />
                             ...
                             {!isUser && (
                               <div className="mt-2 flex gap-2 opacity-0 hover:opacity-100 transition-opacity">
                                  <button onClick={() => copyToClipboard(m.content)} className="flex items-center gap-1 text-[10px] text-white/30 hover:text-[#d4af37]">
                                     <Copy className="w-3 h-3" /> Copy
                                  </button>
                                  <button onClick={regenerateLast} className="flex items-center gap-1 text-[10px] text-white/30 hover:text-[#d4af37]">
                    ...
                    {(typingText) && (
                      <div className="flex flex-col items-start animate-in fade-in">
                          <span className="mb-1 font-mono text-[9px] text-[#d4af37]/50 uppercase tracking-widest px-1">Stream_Incoming</span>
                  ...
                  <div className="p-3 border-t border-white/5 bg-[#0a0a0a]">
                     ...
                     {/* <div className="flex gap-2 mb-3 overflow-x-auto no-scrollbar">
                        {["Pricing Estimate", "Tech Stack", "Schedule Call"].map(txt => (
                           <button 
                             key={txt}
                             onClick={() => setInput(txt)}
                             className="flex-shrink-0 px-3 py-1 text-[10px] font-mono border border-white/10 text-white/40 hover:border-[#d4af37] hover:text-[#d4af37] transition-colors rounded-sm uppercase tracking-wider"
                           >
                             {txt}
                           </button>
                        ))}
                     ...
                     <div className="flex items-end gap-2 bg-white/5 border border-white/10 rounded-sm p-2 focus-within:border-[#d4af37]/50 focus-within:bg-white/10 transition-all">
                        ...
                        <textarea
                           ...
                           className="flex-1 bg-transparent border-none text-sm text-white focus:ring-0 focus:ring-[#d4af37] placeholder:text-white/20 font-sans resize-none max-h-24 py-2 custom-scroll"
                           rows={1}
                        ...
                        <div className="flex gap-1 pb-1">
                           <button
                             ...
                             className={`p-2 rounded-sm transition-all ${isRecording ? "bg-red-500/20 text-red-500" : "text-white/30 hover:text-white"}`}
                           >
                     ...
                     <div className="flex justify-between mt-2 px-1">
                       <span className="text-[9px] font-mono text-white/20 uppercase">Encrypted Connection</span>
